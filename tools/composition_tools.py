...
def register_composition_tools(
    ...
    def validate_deck(
        ...
        - `visual-richness-min-60-pct`: ≥ 60% of body slides must use a
          visual-rich composition (value_cards, capability_grid, stat_cards,
          split_benefits, timeline, diagram, chart, process_steps) rather
          than a text-only one (bullets, solution_detail, section_divider,
          value_props_4). Added iter 44 in response to user feedback "random
          text with no icons / nice visuals" (2026-05-29).
        - `no-text-only-streak-over-2`: max 2 consecutive text-only
          compositions in a row. Three in a row reads as a wall of text.
        - `has-chart-or-diagram`: any deck of ≥ 8 slides must include at
          least one `add_chart_slide` OR `add_diagram_slide`. Pure-text
          decks of that length fail the user's "graphs are limited" bar.
        ...
        # Iter 44: visual-richness, text-only-streak, and has-chart-or-diagram
        # probes. Each mirrors the assess.ts deck-level probe but emits a
        # directive fix_hint (delete index → re-add with named composition)
        # so the model can self-correct inside the validate_deck → fix loop.
        # Driven by user feedback 2026-05-29: even after the iter-43 skill
        # update mandating visual rhythm, the model continued to pick
        # bullets/solution_detail 3-4 times in a row. Skill text alone has
        # ceiling adherence (iter 22 / iter 10 pattern); server-enforced
        # validation closes the gap reliably (iter 25-27 pattern).
        VISUAL_RICH_COMPOSITIONS = {
            "cover", "value_cards", "capability_grid", "stat_cards",
            "split_benefits", "timeline", "diagram", "chart",
            "process_steps", "closing",
        }
        TEXT_ONLY_COMPOSITIONS = {
        ...
        if len(slides) >= 3:
            ...
            if body:
                ...
                # no-text-only-streak-over-2: max 2 consecutive text-only
                # body slides. Three in a row is the wall-of-text pattern.
                longest_text_streak = 0
                current_text_streak = 0
                worst_start = -1
                streak_start_idx = -1
                for i, c in enumerate(body):
                    if c in TEXT_ONLY_COMPOSITIONS:
                        if current_text_streak == 0:
                            streak_start_idx = i
                        current_text_streak += 1
                        if current_text_streak > longest_text_streak:
                            longest_text_streak = current_text_streak
                            worst_start = streak_start_idx
                    else:
                        current_text_streak = 0
                text_streak_ok = longest_text_streak <= 2
                # body[worst_start] is the start of the worst streak; in
                # full deck index, that's worst_start + 1 (because body
                # starts at slide 1).
                streak_slide_indices = (
                    list(range(worst_start + 1, worst_start + 1 + longest_text_streak))
                    if not text_streak_ok else []
                )
                probe_results.append({
                    "name": "no-text-only-streak-over-2",
                    "passed": text_streak_ok,
                    "detail": None if text_streak_ok else (
                        f"Text-only compositions run {longest_text_streak} in a row "
                        f"at slide indices {streak_slide_indices}. Three in a row "
                        f"reads as a wall of text."
                    ),
        ...
        # has-chart-or-diagram: only enforced on 8+ slide decks where a
        # pure-text run is most likely to read as a wall.
        if len(slides) >= 8:
            has_chart_or_diagram = any(c in ("chart", "diagram") for c in compositions)
            probe_results.append({
                "name": "has-chart-or-diagram",
                "passed": has_chart_or_diagram,
                "detail": None if has_chart_or_diagram else (
                    f"Deck has {len(slides)} slides but no chart or diagram. "
                    f"Reading the brief: numeric series / comparison / "
                    f"breakdown → chart; architecture / flow / sequence / "
                    f"dependency → diagram."
                ),
                "fix_hint": None if has_chart_or_diagram else (
